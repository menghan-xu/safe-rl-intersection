...
class IntersectionEnv:
    def __init__(self, target_pos, agent_data_list, config, dt=0.1):
        ...
        # Dynamics: s_{t+1} = Fs_t + Ga_t (Longitudinal only: Y axis)
        ...
        self.state = None 
        self.ego_x = 0.5 # Default, will be overwritten in reset
        self.steps = 0          
        self.episode_steps = 0  

    def reset(self):
        """Resets ego state with random start position and syncs agent."""
        
        # ==========================================
        # 1. Random Start Position (Circle Sampling)
        # ==========================================
        center_x, center_y = 0.5, -1.25
        radius = 0.15
        
        # Uniform sampling within a circle
        # r = R * sqrt(random()) ensures uniform area distribution
        r = radius * np.sqrt(np.random.random())
        theta = np.random.random() * 2 * np.pi
        
        # Calculate start position
        self.ego_x = center_x + r * np.cos(theta) # This X stays fixed during episode
        start_y = center_y + r * np.sin(theta)
        
        # Initialize state [y, v] (using the sampled start_y)
        self.state = torch.tensor([start_y, 0.0], dtype=torch.float32) 
        ...
        # ==========================================
        # 2. Select Agent & Collision Sync
        # ==========================================
        idx = np.random.randint(len(self.agent_data_list))
        self.current_agent_traj = self.agent_data_list[idx]
        
        # Find frame where Agent is closest to Ego's CURRENT path (x = self.ego_x)
        # Previously hardcoded to 0.5, now dynamic
        agent_pos = self.current_agent_traj[:, :2] 
        dist_sq_to_conflict = (agent_pos[:, 0] - self.ego_x)**2 + (agent_pos[:, 1])**2
    ...
    def step(self, action):
        ...
        # 1. Physics Update (Only updates Y and V_y)
        ...
        # 3. Collision Check (Using self.ego_x)
        ego_pos = torch.tensor([self.ego_x, self.state[0]]) # <--- Modified
        ...
        if dist < self.collision_dist:
            ...
            is_success = True
        elif self.state[0] < -6.0 or self.state[0] > 25.0: 
            done = True
    ...
    def _compute_reward_and_cost(self, ego_state, action, agent_info):
        ...
        # Use the sampled ego_x
        xe, ye = self.ego_x, ego_y # <--- Modified
    ...
    def render(self):
        ...
        # Use sampled ego_x for visualization
        ego_pos = [self.ego_x, self.state[0].item()] # <--- Modified
